...
#include <string.h>
#include <arpa/inet.h>
#include <thread>//new cpp standard (portable library)
#include <pthread.h>//posix standard (unix only)
...
int main(int argc, char *argv[]){
    ...
    while (1) { // block on accept() until positive fd or error
        ...
        if ((new_socket = accept(server_socket_fd, (struct sockaddr *)&client_addr, (socklen_t*)&length))<0)
        {
        ...
        if (pthread_create(&thread_id[thread_id_ctr], NULL, client_connection_handler, (void *) &new_socket) < 0)//adds a new thread in the calling process
        {
    ...
    //   pthread_join must be outside the pthread_create loop to create and execute multi threads parallelly. 
    //   if inside, each thread will be created and executed sequentially.
    for (int i = 0; i < MAX_THREADS_AMOUNT; i++){
        pthread_join(thread_id[i], NULL);//waits for the threads specified to terminate..
    }
    pthread_exit(NULL);
